...
const SpecializedStorage: React.FC<SpecializedStorageProps> = ({ activeTab, setActiveTab }) => {
  ...
  const getStorageTypeDisplayName = (type: SpecializedStorageType): string => {
    switch (type) {
  ...
  return (
    ...
    >
      <TabsList className="w-full grid grid-cols-3 bg-space-dark">
        {Object.keys(inventory.specializedStorage).map((key) => (
          <TabsTrigger 
            key={key}
            value={key}
            className="text-xs font-pixel data-[state=active]:bg-space-buttons hover:bg-space-buttons-hover"
          >
            {getStorageTypeDisplayName(key as SpecializedStorageType)}
          </TabsTrigger>
        ))}
      </TabsList>
      
      {Object.entries(inventory.specializedStorage).map(([key, storage]) => (
